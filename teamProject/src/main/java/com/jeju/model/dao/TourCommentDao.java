...
public class TourCommentDao extends SuperDao {

	// 해당 게시물 번호에 달려 있는 댓글 목록을 정렬 시켜 반환해 줍니다.
	public List<TourComment> GetDataByPk(int boardno) throws Exception {
		String sql = " select * from comments " ;
		sql += " where BOARDNO = ? and CATEGORY = 'tour' order by cno asc " ;
		...
		List<TourComment> lists = new ArrayList<TourComment>();
	...
	private TourComment GetBeanData(ResultSet rs) throws Exception{
	...
	public int InsertData(TourComment bean) throws Exception {
		
		int cnt = -1 ;
		
		String sql = " insert into comments (cno, id, boardno, category, content, regdate) " ;
		sql += " values(seqcomment.nextval, ?, ?, 'tour', ?, sysdate) " ; // 'food' 대신 게시판별로 카테고리 입력
